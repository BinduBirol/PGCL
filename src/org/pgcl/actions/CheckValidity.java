...
import org.pgcl.models.UserService;
	...
	{		
		if(this.isPostRequest() && user.getUserId()!=null && user.getPassword()!=null)
		{
			...
			if(loggedInUser!=null )
			{
				...
				session.put("USER_BANK_LIST", BankBranchService.getBankList(0,0,"bank.status=1 and AREA_ID='"+loggedInUser.getArea_id()+"'","BANK_NAME","ASC",0));
				session.put("BANK_USER", BankBranchService.getUserBank(0,0,"bank.status=1 and BRANCH_ID="+loggedInUser.getBranch_id()+" and AREA_ID= '"+loggedInUser.getArea_id()+"'","BANK_NAME","ASC",0));	
