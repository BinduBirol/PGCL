package org.pgcl.models;
	...
	public class AdjustmentCollectionService {
		...
		public CollectionDTO getBillingInfo(String customer_id,String bill_month,String bill_year,String collection_date)
		{
			...
			//{
				//if(bill_month.equalsIgnoreCase("") && bill_year.equalsIgnoreCase("")){
					...
					sql=" select BILL_MONTH,BILL_YEAR,BILL_ID,STATUS,BILLED_AMOUNT+HHV_NHV_ADJ_QNT BILLED_AMOUNT ,COLLECTED_AMOUNT COLLECTED_BILLED_AMOUNT,ACTUAL_SURCHARGE ACTUAL_SURCHARGE_CAL,COLLECTED_SURCHARGE,PAYABLE_AMOUNT ACTUAL_PAYABLE_AMOUNT_CAL,TOTAL_COLLECTED_AMOUNT COLLECTED_PAYABLE_AMOUNT,null surcharge_per_coll,DOUBLE_BURNER_QNT double_burner_qnt,VAT_REBATE_AMOUNT from SALES_ADJUSTMENT Where Status=1 And Customer_Id = ? "+
					" AND  Bill_Id in ( Select MIN(to_number(bill_id)) from SALES_ADJUSTMENT Where Status=1 And Customer_Id=? ) " ;
		...
		public ResponseDTO saveBillCollection(CollectionDTO collection,boolean mobilePhoneUpdate)
		{
			...
			String sql_update_metered="Update SALES_ADJUSTMENT Set Status=2,COLLECTED_HHV_NHV=? Where Bill_Id=?";
			...
			String bank_book_info_select="Select 0 Payable_Amount, 0 GAS_BILL,METER_RENT,ACTUAL_SURCHARGE SURCHARGE_AMOUNT  From SALES_ADJUSTMENT Where BILL_ID= ? ";
			...
			try
				{
					...
					if(isMeteredStatus.equalsIgnoreCase("1")){
						...
						stmt.setDouble(1,collection.getBilled_amount());					
