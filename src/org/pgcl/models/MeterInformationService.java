...
public class MeterInformationService {
	...
	{		
		...
		ResultSet r = null;
			try
			{
				...
				String conn_update_sql="Update customer_connection set STATUS=1 where customer_id=?";
	...
	public PreparedStatement getInsertStatementForNewMeter(Connection conn,CustomerMeterDTO meter){
		...
		String sql_meter=" Insert Into CUSTOMER_METER(CUSTOMER_ID,METER_ID,METER_SL_NO,METER_MFG,METER_YEAR,MEASUREMENT_TYPE, " +
		 ...
		 " PRESSURE,TEMPERATURE,METER_RENT,INSTALLED_BY,INSTALLED_DATE,STATUS,REMARKS) " +
	...
	{		
		...
		String sql_all_data=" Update CUSTOMER_METER Set MEASUREMENT_TYPE=?,EVC_SL_NO=?,EVC_MODEL=?, " +
				   " EVC_YEAR=?,METER_TYPE=?,G_RATING=?,CONN_SIZE=?,MAX_READING=?,INSTALLED_BY=?,REMARKS=?," +
		...
		try
		{
			...
			stmt.setString(10,meter.getRemarks());
			
			if(editResponse.isResponse()==true){
				stmt.setString(11,meter.getMeter_sl_no());
				stmt.setString(12,meter.getMeter_mfg());
				stmt.setString(13,meter.getMeter_year());
				stmt.setString(14,meter.getIni_reading());
				stmt.setString(15,meter.getPressure());
				stmt.setString(16,meter.getTemperature());			
				stmt.setString(17,meter.getInstalled_date());
				stmt.setString(18,meter.getMeter_id());
			}
			else
				stmt.setString(11,meter.getMeter_id());
