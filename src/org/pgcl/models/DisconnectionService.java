...
public class DisconnectionService {
	...
	public ResponseDTO saveMeterDisconnInfo(DisconnectDTO disconn,MeterReadingDTO reading)
	{		
		...
		String sqlUpdate=" Update CUSTOMER_METER set status=? where customer_id=? and meter_ID=?";
			...
			try
			{
				coll_stmt = conn.prepareStatement("Select SQN_METER_READING.nextval reading_id from dual");
				...
				stmt.setFloat(5,reading.getRate());
				...
				stmt.setFloat(9,reading.getPrev_reading());
				stmt.setString(10,reading.getPrev_reading_date());
				stmt.setFloat(11,reading.getCurr_reading());
				stmt.setString(12,disconn.getDisconnect_date());
				stmt.setFloat(13,reading.getDifference());
				stmt.setFloat(14,reading.getHhv_nhv());
				...
				stmt.setFloat(19,reading.getMeter_rent());
				stmt.setFloat(20,reading.getPressure());
				stmt.setDouble(21,reading.getPressure_factor());
				stmt.setFloat(22,reading.getTemperature());
				...
				if(customer==null)
					customer=customerService.getCustomerInfo(reading.getCustomer_id());
	...
	public ResponseDTO updateMeterDisconnInfo(DisconnectDTO disconn,MeterReadingDTO reading)
	{		
		ResponseDTO response=new ResponseDTO();
		TransactionManager transactionManager=new TransactionManager();
		Connection conn = transactionManager.getConnection();
	...
	public ResponseDTO validateDisconnInfo(DisconnectDTO disconn,MeterReadingDTO reading){
		ResponseDTO response=new ResponseDTO();
		BillingService bService=new BillingService();
		BillingMeteredDTO bill= bService.getBillStatus(reading.getBilling_month(), reading.getBilling_year(), disconn.getCustomer_id());
		
		if(bill==null){
			
			if(isValidDisconnDate(reading.getMeter_id(),disconn.getDisconnect_date())==true)
				response.setResponse(true);
			else
			{
				response.setResponse(false);
	...
	public ArrayList<DisconnectDTO> getMeterDisconnectionList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
	...
	public DisconnectDTO getMeterDisconnectionInfo(String disconnection_id,String meter_id)
	{		
		DisconnectDTO disconn=null;
		...
		ResultSet r = null;
			try
			{
				...
				if (r.next())
				{
					...
					customer=customerService.getCustomerInfo(r.getString("DISCONN_CUSTOMER_ID"));
	...
	{		
		...
		String sqlUpdate=" Update CUSTOMER_CONNECTION set status="+ConnectionStatus.DISCONNECTED.getId()+" where customer_id=?";
	...
	public DisconnectDTO getNonMeterDisconnectionInfo(String disconnection_id,String customer_id)
	{		
		DisconnectDTO disconn=null;
		Connection conn = ConnectionManager.getConnection();
	...
	public ArrayList<DisconnectDTO> getNonMeterDisconnectionList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		DisconnectDTO disConn=null;
		ArrayList<DisconnectDTO> disConnList=new ArrayList<DisconnectDTO>();
