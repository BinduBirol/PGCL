...
import org.pgcl.enums.BurnerCategory;
...
public class TariffService {
	...
	public String addTariff(String data)
	{
		...
		Connection conn = ConnectionManager.getConnection();
		String sql=" Insert Into MST_TARIFF(TARIFF_ID,TARIFF_NO,CUSTOMER_CATEGORY_ID,METER_STATUS,BURNER_CATEGORY,PRICE,PB, VAT, SD, PDF, BAPEX,  WELLHEAD, DWELLHEAD, TRNSMISSION,GDFUND,DISTRIBUTION,AVALUE,OTHER1,EFFECTIVE_FROM,DESCRIPTION,IS_DEFAULT)" +
		...
		PreparedStatement stmt = null;
			try
			{
				...
				stmt.setDouble(i++,tariff.getPrice());
				stmt.setDouble(i++,tariff.getPb());
				stmt.setDouble(i++,tariff.getVat());
				stmt.setDouble(i++,tariff.getSd());
				stmt.setDouble(i++,tariff.getPdf());
				stmt.setDouble(i++,tariff.getBapex());
				stmt.setDouble(i++,tariff.getWellhead());
				stmt.setDouble(i++,tariff.getDwellhead());
				stmt.setDouble(i++,tariff.getTrasmission());
				stmt.setDouble(i++,tariff.getGdfund());
				stmt.setDouble(i++,tariff.getDistribution());
				stmt.setDouble(i++,tariff.getAvalue());
				stmt.setDouble(i++,tariff.getOthers1());
	...
	public String updateTariff(String data)
	{
		...
		PreparedStatement stmt = null;
			try
			{
				...
				stmt.setDouble(4,tariffDTO.getPrice());
	...
	public String getTariffRateForDomestic(String customer_id) {
		...
		try {
			...
			while (r.next()) {
				...
				BurnerPriceForDomesticPVT.append("#");
			}
			BurnerPriceForDomesticPVT.deleteCharAt(BurnerPriceForDomesticPVT.length() - 1);
