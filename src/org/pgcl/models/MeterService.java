...
public class MeterService {
	...
	public ArrayList<MeterTypeDTO> getMeterTypeList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		String cKey="METER_TYPE_"+Utils.constructCacheKey(index,offset,total,whereClause,sortFieldName,sortOrder);
		ArrayList<MeterTypeDTO> meterTypeList=CacheUtil.getListFromCache(cKey,MeterTypeDTO.class);
		...
		if(total==0)
				  sql = " Select * from MST_METER_TYPE  "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" ";
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
				  	  " ( Select * from MST_METER_TYPE "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery+			  	  
				  	  "    )tmp1 " +
				  	  "    )tmp2   " +
	...
	{
		return getMeterTypeList(0, 0,Utils.EMPTY_STRING,Utils.EMPTY_STRING,Utils.EMPTY_STRING,0);
	}
	...
	public String addMeterType(String data)
	{
		...
		PreparedStatement stmt = null;
	 	...
	 	if(response==1)
	 		return Utils.getJsonString(AC.STATUS_OK, AC.MSG_CREATE_OK_PREFIX+AC.MST_METER_TYPE);
	 	else
	 		return Utils.getJsonString(AC.STATUS_ERROR, AC.MSG_CREATE_ERROR_PREFIX+AC.MST_METER_TYPE);
	...
	public String updateMeterType(String data)
	{
		...
		PreparedStatement stmt = null;
	 	...
	 	if(response==1)
	 		return Utils.getJsonString(AC.STATUS_OK, AC.MSG_UPDATE_OK_PREFIX+AC.MST_METER_TYPE);
	 	else
	 		return Utils.getJsonString(AC.STATUS_ERROR, AC.MSG_UPDATE_ERROR_PREFIX+AC.MST_METER_TYPE);
	...
	public String deleteMeterType(String meterTypeId)
	{
		...
		PreparedStatement stmt = null;
			...
			return Utils.getJsonString(AC.STATUS_ERROR, e.getMessage());
	 		...
	 		if(response==1)
		 		return Utils.getJsonString(AC.STATUS_OK, AC.MSG_DELETE_OK_PREFIX+AC.MST_METER_TYPE);
		 	else
		 		return Utils.getJsonString(AC.STATUS_ERROR, AC.MSG_DELETE_ERROR_PREFIX+AC.MST_METER_TYPE);
	...
	public ArrayList<CustomerMeterDTO> getMeterList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		String customer_id=whereClause.split("=")[1].replaceAll("'", "");
		return getCustomerMeterList(customer_id.trim(),Utils.EMPTY_STRING,Utils.EMPTY_STRING);
	}
	public ArrayList<CustomerMeterDTO> getCustomerMeterList(String customer_id,String meter_id,String where_clause)
	{		
		...
		if(meter_id==null || meter_id.equalsIgnoreCase(Utils.EMPTY_STRING))
			   sql= "Select tmp1.customer_id,meter_id,meter_sl_no,meter_mfg,mfg_name,meter_year,measurement_type, " +
			   		...
			   		" from  " +
			   		" ( " +
			   		" Select * from customer_meter  WHERE customer_meter.customer_id = ? " +where_clause+" )tmp1  " +
				...
				sql=" SELECT * " +
				"        FROM (Select tmp1.customer_id,meter_id,meter_sl_no,meter_mfg,mfg_name,meter_year,measurement_type, " +
				"        evc_sl_no,evc_model,evc_year,meter_type,type_name meter_type_name,g_rating,rating_name g_rating_name,conn_size,max_reading, " +
				"        ini_reading,pressure,temperature,meter_rent,to_char(installed_date,'DD-MM-YYYY') installed_date,installed_by,tmp1.status,remarks,GETPREVREADINGINFO(meter_id) reading_prev_info  " +
				"        From  " +
				"        ( " +
				"            Select * from customer_meter  WHERE meter_id = ? " +where_clause+" )tmp1  " +
				"        Left Outer Join MST_METER_MFG on tmp1.meter_mfg=MST_METER_MFG.MFG_ID " +
				"        Left Outer Join MST_EVC_MODEL on tmp1.evc_model=MST_EVC_MODEL.MODEL_ID " +
				"        Left Outer Join MST_METER_TYPE on tmp1.meter_type=MST_METER_TYPE.TYPE_ID " +
				"        Left Outer Join MST_METER_GRATING on tmp1.g_rating=MST_METER_GRATING.RATING_ID " +
		...
		MeterReadingDTO reading=null;
			try
			{
				...
				{				
					...
					meter.setMeasurement_type(MeterMeasurementType.values()[r.getInt("MEASUREMENT_TYPE")]);
					meter.setMeasurement_type_str(String.valueOf(MeterMeasurementType.values()[r.getInt("MEASUREMENT_TYPE")].getId()));
					meter.setMeasurement_type_name(MeterMeasurementType.values()[r.getInt("MEASUREMENT_TYPE")].getLabel());
					...
					meter.setInstalled_by_str(EmployeeService.getEmpNameFromEmpId(r.getString("INSTALLED_BY")));		
					
					meter.setRemarks(r.getString("REMARKS"));
					meter.setStatus(MeterStatus.values()[r.getInt("STATUS")]);
					meter.setStatus_str(String.valueOf(MeterStatus.values()[r.getInt("STATUS")].getId()));
					meter.setStatus_name(MeterStatus.values()[r.getInt("STATUS")].getLabel());
	...
	public ArrayList<CustomerMeterDTO> getDisconnectedMeterList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{		
		ArrayList<CustomerMeterDTO> meterList=new ArrayList<CustomerMeterDTO>();
		Connection conn = ConnectionManager.getConnection();
		String sql="";
		sql=" Select CUSTOMER_METER.*,to_char(INSTALLED_DATE,'DD-MM-YYYY') INSTALLED_DATE_FORMATTED from CUSTOMER_METER Where "+whereClause ;
	...
	public DisconnectDTO getLatestDisconnectInfo(String customer_id,String meter_no)
	{
	...
	public ArrayList<MeterMfgDTO> getMfgList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		String cKey="METER_MFG_"+Utils.constructCacheKey(index,offset,total,whereClause,sortFieldName,sortOrder);
		ArrayList<MeterMfgDTO> mfgList=CacheUtil.getListFromCache(cKey,MeterMfgDTO.class);
		if(mfgList!=null)
			return mfgList;
		else
		...
		if(sortFieldName!=null && !sortFieldName.equalsIgnoreCase(""))
			orderByQuery=" ORDER BY "+sortFieldName+" " +sortOrder+" ";
		if(total==0)
				  sql = " Select * from MST_METER_MFG  "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" ";
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
				  	  " ( Select * from MST_METER_MFG "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery+			  	  
				  	  "    )tmp1 " +
				  	  "    )tmp2   " +
				  	  "  Where serial Between ? and ? ";
	...
	{
		return getMfgList(0, 0,Utils.EMPTY_STRING,Utils.EMPTY_STRING,Utils.EMPTY_STRING,0);
	}
	
	public ArrayList<MeterGRatingDTO> getGRatingList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		String cKey="GRATING_"+Utils.constructCacheKey(index,offset,total,whereClause,sortFieldName,sortOrder);
		ArrayList<MeterGRatingDTO> gRatingList=CacheUtil.getListFromCache(cKey,MeterGRatingDTO.class);
		if(gRatingList!=null)
			return gRatingList;
		else
		...
		if(sortFieldName!=null && !sortFieldName.equalsIgnoreCase(""))
			orderByQuery=" ORDER BY "+sortFieldName+" " +sortOrder+" ";
		if(total==0)
				  sql = " Select * from MST_METER_GRATING  "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery;
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
				  	  " ( Select * from MST_METER_GRATING "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery+			  	  
				  	  "    )tmp1 " +
				  	  "    )tmp2   " +
				  	  "  Where serial Between ? and ? ";
	...
	{
		return getGRatingList(0, 0,Utils.EMPTY_STRING,Utils.EMPTY_STRING,Utils.EMPTY_STRING,0);
	}	
	
		
	
	public ArrayList<EVCModelDTO> getEvcModelList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		String cKey="EVC_MODEL_"+Utils.constructCacheKey(index,offset,total,whereClause,sortFieldName,sortOrder);
		ArrayList<EVCModelDTO> evcModelLit=CacheUtil.getListFromCache(cKey,EVCModelDTO.class);
		if(evcModelLit!=null)
			return evcModelLit;
		else
		...
		if(sortFieldName!=null && !sortFieldName.equalsIgnoreCase(""))
			orderByQuery=" ORDER BY "+sortFieldName+" " +sortOrder+" ";
		if(total==0)
				  sql = " Select * from MST_EVC_MODEL  "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery;
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
				  	  " ( Select * from MST_EVC_MODEL  "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery+			  	  
				  	  "    )tmp1 " +
				  	  "    )tmp2   " +
				  	  "  Where serial Between ? and ? ";
	...
	public static ResponseDTO isMeterBasicInfoChangeValid(String meter_id){
		...
		return MeterService.isMeterDeleteValid(meter_id); //Because both share common logic
	...
	public static ResponseDTO isMeterDeleteValid(String meter_id){
		...
		Connection conn = ConnectionManager.getConnection();
		String sql=" Select count(READING_ID) TOTAL from METER_READING Where Meter_Id=? and reading_purpose!="+ReadingPurpose.NEW_METER.getId();
		PreparedStatement stmt = null;
		try
		{
			stmt = conn.prepareStatement(sql);
	...
	public ArrayList<MeterRepairmentDTO> getRepairmentList(int index, int offset,String whereClause,String sortFieldName,String sortOrder,int total)
	{
		String cKey="REPAIRMENT_"+Utils.constructCacheKey(index,offset,total,whereClause,sortFieldName,sortOrder);
		ArrayList<MeterRepairmentDTO> repairmentList=CacheUtil.getListFromCache(cKey,MeterRepairmentDTO.class);
		...
		if(total==0)
				  sql = " Select cp.CUSTOMER_ID,FULL_NAME,PID,repairment.METER_ID,METER_SL_NO,PREV_READING,To_Char(PREV_READING_DATE,'DD-MM-YYYY HH24:MI') PREV_READING_DATE,CURR_READING,To_Char(CURR_READING_DATE,'DD-MM-YYYY HH24:MI') CURR_READING_DATE,REPAIRED_BY,repairment.REMARKS,READING_ID " +
				  	  "  From METER_REPAIRMENT repairment,CUSTOMER_PERSONAL_INFO cp,CUSTOMER_METER meter Where cp.customer_id=meter.customer_id and meter.meter_id=repairment.meter_id   "+(whereClause.equalsIgnoreCase("")?"":("Where ( "+whereClause+")"))+" "+orderByQuery;
		else
				  sql=" Select * from ( " +
				  	  " Select rownum serial,tmp1.* from " +
				  	  ...
				  	  "  From METER_REPAIRMENT repairment,CUSTOMER_PERSONAL_INFO cp,CUSTOMER_METER meter Where cp.customer_id=meter.customer_id and meter.meter_id=repairment.meter_id "+whereClause+" "+orderByQuery+			  	  
				  	  "    )tmp1 " +
				  	  "    )tmp2   " +
				  	  "  Where serial Between ? and ? ";
	...
	public static MeterRepairmentDTO getRepairInfo(String repair_id)
	{
		...
		MeterRepairmentDTO repair=(MeterRepairmentDTO)CacheUtil.getObjFromCache(cKey);
	...
	public static ResponseDTO saveMeterReplacement(String meter_id,CustomerMeterDTO meter)
	{
		ResponseDTO response=new ResponseDTO();
		Connection conn = ConnectionManager.getConnection();
		...
		try {
            ...
            stmt = (OracleCallableStatement) conn.prepareCall("{ call saveMeterReplacement(?,?,?,?,?,?,?,?,?,?)  }");
