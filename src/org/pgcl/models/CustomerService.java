...
import org.pgcl.dto.CustomerGridDTO;
...
public class CustomerService {
	...
	public static ResponseDTO createNewCustomer(CustomerDTO customer,CustomerPersonalDTO personalInfo,AddressDTO addressInfo)
	{
		...
		String customerSql=" Insert Into CUSTOMER(CUSTOMER_ID,APP_SL_NO,CUSTOMER_CATEGORY,AREA,ZONE) Values(?,?,?,?,?)";
		String personalSql=" Insert Into CUSTOMER_PERSONAL_INFO(CUSTOMER_ID,FULL_NAME,FATHER_NAME,MOTHER_NAME,GENDER,EMAIL,PHONE, " +
						   " MOBILE,FAX,FREEDOM_FIGHTER,NATIONAL_ID,PASSPORT_NO,LICENSE_NUMBER,VAT_REG_NO,PROPRIETOR_NAME,ORGANIZATION_NAME) " +
						   " Values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
//					customer.setZone(r.getString("ZONE"));
					...
					connectionInfo.setSingle_burner_qnt(r.getInt("SINGLE_BURNER_QNT"));
					connectionInfo.setDouble_burner_qnt(r.getInt("DOUBLE_BURNER_QNT"));
					connectionInfo.setDouble_burner_qnt_billcal(r.getFloat("DOUBLE_BURNER_QNT_BILLCAL"));
					connectionInfo.setPemanently_disconnected_burner_qnt(r.getInt(("PDISCONNECTED_BURNER_QNT")));
					connectionInfo.setTemporary_disconnected_burner_qnt(r.getInt(("TDISCONNECTED_BURNER_QNT")));
	...
	{  	
		...
		String newOwnershipInfo=" Update CUSTOMER_PERSONAL_INFO Set FULL_NAME=?,FATHER_NAME=?,MOTHER_NAME=?,GENDER=?,MOBILE=?,PHONE=?,FAX=?,NATIONAL_ID=?,PASSPORT_NO=? Where customer_id=?";
		PreparedStatement stmt = null;
		try
		{
