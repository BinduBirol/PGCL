...
public class NonMeterConnectionReport extends BaseAction {
	...
	private void getMeterCustomerReconnectionInfo(Document document) throws DocumentException
	{
		...
		for(int i=0;i<expireListSize;i++)
		{
			...
			ptable = new PdfPTable(12);
			ptable.setWidthPercentage(100);
			ptable.setWidths(new float[]{15,75,25,25,25,30,30,30,30,30,30,30});
			...
			pcell=new PdfPCell(new Paragraph("Percial Bill",ReportUtil.f9B));
			pcell.setRowspan(2);
			pcell.setMinimumHeight(18f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			...
			pcell=new PdfPCell(new Paragraph("Contractual",ReportUtil.f9B));
			pcell.setMinimumHeight(18f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
	...
	ArrayList<NonMeterReportDTO> disconnCustomerList=new ArrayList<NonMeterReportDTO>();
		...
		try {
			...
			String defaulterSql="select MR.CUSTOMER_ID,mci.FULL_NAME,mci.CATEGORY_ID,mci.CATEGORY_NAME,METER_SL_NO,CONNECTION_DATE INSTALLED_DATE,INSTALLED_DATE INSTALLED_DATE_ORDER,PMIN_LOAD,PMAX_LOAD,MR.METER_ID " +
	...
	//PreparedStatement ps1=null;
		...
		try {
			...
			String defaulterSql=" " +
					...
					"         NEW_DOUBLE_BURNER_QNT BURNER_QNT, " +
					"         to_char(EFFECTIVE_DATE,'dd-MM-YYYY') EFFECTIVE_DATE,EFFECTIVE_DATE  EFFECTIVE_DATE_ORDER ," +
					"         calculate_partial_bill (bqcM.CUSTOMER_ID,NEW_DOUBLE_BURNER_QNT,'C', " +
					...
					"                 AND TO_CHAR (EFFECTIVE_DATE, 'mm') =" +bill_month+
        	...
        	while(resultSet.next())
        	{
        		...
        		loadIncraseDTO.setBurner_qnt(resultSet.getInt("BURNER_QNT"));
