...
public class SGFLSecond extends BaseAction {
	...
	public String execute() throws Exception
	{
		...
		String fileName="SGFL_Report.pdf";
		...
		try{
			...
			double netPayableAmt=0.0;
			
			sgfllList=getBGFCLCredit();
			int listSize=sgfllList.size();
			
			for (int i = 0; i < listSize; i++) {
				netPayableAmt = sgfllList.get(i).getNetPayable();
				...
				if(netPayableAmt>0.0){
					balance=balance-sgfllList.get(i).getNetPayable()-sgfllList.get(i).getIncomeTax();
				}else{
				...
				if(netPayableAmt>0.0){
					...
					totalNetPabable=totalNetPabable+sgfllList.get(i).getNetPayable();
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(sgfllList.get(i).getNetPayable()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					
					totalIncomeTax=totalIncomeTax+sgfllList.get(i).getIncomeTax();
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(sgfllList.get(i).getIncomeTax()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
					...
					totalDebit=totalDebit+sgfllList.get(i).getNetPayable()+sgfllList.get(i).getIncomeTax();
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(sgfllList.get(i).getNetPayable()+sgfllList.get(i).getIncomeTax()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
					...
					totalVatRevate=totalVatRevate+sgfllList.get(i).getVatRebate();
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(sgfllList.get(i).getVatRebate()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
	...
	private ArrayList<JournalVoucherDTO>getBGFCLCredit(){
		...
		try {
			...
			String defaulterSql="select month,year, WELLHEAD, sd, vat,TOTAL_SGFL, TO_CHAR(LASTDAY) LASTDAY from( " +
					...
					"from GAS_PURCHASE_SUMMARY where year||lpad(month,2,0) between 201507 and 201606 " +
					"union all " +
					"select month,year,WELLHEAD, sd,vat, null TOTAL_SGFL,LAST_DAY(TO_Date(month||'-'||year, 'MM-YYYY')) LASTDAY " +
					"from MARGIN_SGFL where year||lpad(month,2,0) between 201507 and 201606 " +
