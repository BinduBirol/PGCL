...
import org.pgcl.dto.BankBookDTO;
...
public class BankBookRevenue extends BaseAction {
	...
	public String execute() throws Exception
	{
		...
		try{
			...
			Chunk chunk1 = new Chunk("Revenue Section :",ReportUtil.f9B);
			...
			pcell = new PdfPCell(new Paragraph("STD A/C : "+accountInfo.getAccount_no()+", "+accountInfo.getBank_name()+", "+accountInfo.getBranch_name(),ReportUtil.f9B));
			pcell.setMinimumHeight(18f);
			pcell.setColspan(3);
			...
			if(account_no.equals("1001")){
				pcell = new PdfPCell(new Paragraph("Area",ReportUtil.f11B));
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setColspan(1);
				debitTable.addCell(pcell);
	...
	try {
		...
		if(account_no.equals("2916-01-0000300") || account_no.equals("01787685899-0501")){
			...
			transaction_sql="SELECT DECODE(AREA_ID,'01','SIRAJGANJ','02','BAGHABARI','03','PABNA','04','ISHWARDI','05','BOGURA','06','SHAHJADPUR','07','BERA','08','SHANTHIA','09','ULLAHPARA','10','RAJSHAHI') TRANS_DATE,DEBIT, " +
					"       METER_RENT, " +
					"       SURCHARGE, " +
					"       DECODE (cat, " +
					"               '01', 'DOMESTIC PVT.', " +
					...
					"       0 Transfer_Amount, " +
					"       0 HHV_NHV_BILL " +
					"  FROM (  SELECT SUBSTR (CUSTOMER_ID, 1, 2) AREA_ID, SUM (NVL (DEBIT, 0)) DEBIT, " +
					"                 SUM (NVL (METER_RENT, 0)) METER_RENT, " +
					"                 SUM (NVL (SURCHARGE, 0)) SURCHARGE, " +
					"                 SUBSTR (customer_id, 3, 2) cat " +
					"            FROM BANK_ACCOUNT_LEDGER " +
					"           WHERE ACCOUNT_NO = '"+account_no+"' " + wClause+
					"        GROUP BY SUBSTR (CUSTOMER_ID, 1, 2),SUBSTR (customer_id, 3, 2)) " +
					"        order by AREA_ID " ;
			
		}else{		
		
		 transaction_sql="SELECT TO_CHAR (TRANS_DATE, 'dd-mm-rrrr') TRANS_DATE,TRANS_DATE TRANS_DATE1, PARTICULARS, DEBIT, METER_RENT, SURCHARGE, interest,Miscellaneous, Transfer_Amount,  " +
				 "HHV_NHV_BILL FROM (SELECT TRANS_DATE,PARTICULARS,DEBIT,METER_RENT,SURCHARGE,interest,Miscellaneous,Transfer_Amount, HHV_NHV_BILL FROM ( " +
				 "SELECT PARTICULARS,MAX (TRANS_DATE) TRANS_DATE,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE,SUM (INTEREST) INTEREST,  " +
				 "SUM (MISCELLANEOUS) MISCELLANEOUS,SUM (TRANSFER_AMOUNT) TRANSFER_AMOUNT,SUM (HHV_NHV_BILL) HHV_NHV_BILL FROM (SELECT TRANS_DATE,  " +
				 "DECODE ( cat, '01', 'Accounts Receivable Domestic Pvt.', '02', 'Accounts Receivable Domestic Govt.','03', 'Accounts Receivable Commercial Pvt.',  " +
				 "'04', 'Accounts Receivable Commercial Govt.','05', 'Accounts Receivable Industrial Pvt.','06', 'Accounts Receivable Industrial Govt.',  " +
				 "'07', 'Accounts Receivable Captive Pvt.','08', 'Accounts Receivable Captive Govt.','09', 'Accounts Receivable C.N.G Pvt.',  " +
				 "'10', 'Accounts Receivable C.N.G Govt.','11', 'Accounts Receivable Power Pvt.','12', 'Accounts Receivable Power Govt.')  PARTICULARS,  " +
				 "DEBIT,METER_RENT,SURCHARGE,NULL interest,NULL Miscellaneous,NULL Transfer_Amount,HHV_NHV_BILL FROM ( " +
				 "SELECT TRANS_DATE,cat,DEBIT,METER_RENT,SURCHARGE+nvl(ADJUSTMENT_AMOUNT,0) SURCHARGE,CATEGORY,HHV_NHV_BILL FROM (  " +
				 ...
				 "FROM BANK_ACCOUNT_LEDGER  WHERE TRANS_TYPE = 1 AND CUSTOMER_ID IS NOT NULL  " +
				 "AND ACCOUNT_NO = '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, SUBSTR (CUSTOMER_ID, 3, 2),ACCOUNT_NO) tab1,  " +
				 "(SELECT sum(nvl (HHV_NHV_BILL,0)) HHV_NHV_BILL,SUM(nvl (ADJUSTMENT_AMOUNT,0)) ADJUSTMENT_AMOUNT,CATEGORY,ACCOUNT_NO,COLLECTION_DATE from(  " +
				 "SELECT SUM(nvl (HHV_NHV_BILL,0)) HHV_NHV_BILL,SUM(nvl (ADJUSTMENT_AMOUNT,0)) ADJUSTMENT_AMOUNT, SUBSTR (CUSTOMER_ID, 3, 2) CATEGORY,ACCOUNT_NO,COLLECTION_DATE   " +
				 "FROM SUMMARY_MARGIN_PB pb, BILL_COLLECTION_METERED bcm   " +
				 "WHERE  pb.bill_id = BCM.BILL_ID AND ACCOUNT_NO= '"+account_no+"'  "+wClause2+
				 "GROUP BY SUBSTR (CUSTOMER_ID, 3, 2),ACCOUNT_NO,COLLECTION_DATE  " +
				 "union all  " +
				 "select sum(nvl(collected_hhv_nhv,0)) HHV_NHV_BILL,null ADJUSTMENT_AMOUNT,substr(sa.customer_id,3,2) Category,ACCOUNT_NO,COLLECTION_DATE from sales_adjustment sa,bill_collection_metered bcm  " +
				 "where sa.bill_id=bcm.bill_id  AND ACCOUNT_NO= '"+account_no+"'  "+wClause2+
				 "GROUP BY substr(sa.customer_id,3,2),ACCOUNT_NO,COLLECTION_DATE)  " +
				 "group by CATEGORY,ACCOUNT_NO,COLLECTION_DATE) tab2  " +
				 "WHERE tab1.cat = tab2.CATEGORY(+)  " +
				 "and tab1.ACCOUNT_NO=tab2.ACCOUNT_NO(+)  " +
				 "and tab1.TRANS_DATE=tab2.COLLECTION_DATE(+)))  " +
				 "GROUP BY PARTICULARS)  " +
				 "UNION ALL  " +
				 "SELECT TRANS_DATE,PARTICULARS,DEBIT,METER_RENT,SURCHARGE,NULL interest,NULL Miscellaneous,DEBIT Transfer_Amount,NULL HHV_NHV_BILL FROM ( " +
				 "SELECT TRANS_DATE,PARTICULARS,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE  " +
				 "FROM BANK_ACCOUNT_LEDGER WHERE TRANS_TYPE = 4 AND DEBIT > 0 AND ACCOUNT_NO =  '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, PARTICULARS order by PARTICULARS,TRANS_DATE)  " +
				 "UNION ALL  " +
				 "SELECT TRANS_DATE,PARTICULARS,DEBIT,METER_RENT,SURCHARGE,DEBIT interest,NULL Miscellaneous,NULL Transfer_Amount,NULL HHV_NHV_BILL FROM ( " +
				 "SELECT TRANS_DATE,PARTICULARS,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE FROM BANK_ACCOUNT_LEDGER  " +
				 "WHERE TRANS_TYPE = 5  AND DEBIT > 0  AND ACCOUNT_NO =  '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, PARTICULARS)  " +
				 "UNION ALL  " +
				 "SELECT TRANS_DATE,PARTICULARS,DEBIT,METER_RENT,SURCHARGE,DEBIT interest,NULL Miscellaneous,NULL Transfer_Amount,NULL HHV_NHV_BILL FROM ( " +
				 "SELECT TRANS_DATE,PARTICULARS,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE FROM BANK_ACCOUNT_LEDGER  " +
				 "WHERE TRANS_TYPE = 3 AND DEBIT > 0 AND PARTICULARS LIKE '%Interest%' AND ACCOUNT_NO =  '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, PARTICULARS)  " +
				 "UNION ALL  " +
				 "SELECT TRANS_DATE,PARTICULARS,DEBIT,METER_RENT,SURCHARGE,NULL interest,DEBIT Miscellaneous,NULL Transfer_Amount,NULL HHV_NHV_BILL FROM ( " +
				 "SELECT TRANS_DATE,PARTICULARS,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE FROM BANK_ACCOUNT_LEDGER  " +
				 "WHERE TRANS_TYPE = 3  AND DEBIT > 0 AND PARTICULARS LIKE '%Others%' AND ACCOUNT_NO =  '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, PARTICULARS)  " +
				 "UNION ALL  " +
				 "SELECT TRANS_DATE,PARTICULARS,DEBIT DEBIT, METER_RENT,SURCHARGE,NULL interest, NULL Miscellaneous,NULL Transfer_Amount,NULL HHV_NHV_BILL FROM ( " +
				 "SELECT TRANS_DATE,PARTICULARS,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE FROM BANK_ACCOUNT_LEDGER  " +
				 "WHERE  TRANS_TYPE = 7 AND DEBIT > 0 AND ACCOUNT_NO =  '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, PARTICULARS)  " +
				 "UNION all  " +
				 "SELECT TRANS_DATE,PARTICULARS,DEBIT DEBIT,METER_RENT,SURCHARGE,NULL interest,NULL Miscellaneous,NULL Transfer_Amount,NULL HHV_NHV_BILL  " +
				 "FROM (SELECT TRANS_DATE,PARTICULARS,SUM (DEBIT) DEBIT,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE  " +
				 "FROM BANK_ACCOUNT_LEDGER  WHERE TRANS_TYPE = 666 AND DEBIT > 0 AND ACCOUNT_NO =  '"+account_no+"'  "+wClause+
				 "GROUP BY TRANS_DATE, PARTICULARS))  " +
				 ...
				 /*"  SELECT TO_CHAR (TRANS_DATE, 'dd-mm-rrrr') TRANS_DATE,TRANS_DATE TRANS_DATE1, " +
				"         PARTICULARS, " +
				"         DEBIT, " +
				"         METER_RENT, " +
				"         SURCHARGE, " +
				...
				"                                           tab1, " +
				"                                           (SELECT sum(nvl (HHV_NHV_BILL,0)) HHV_NHV_BILL,CATEGORY,ACCOUNT_NO,COLLECTION_DATE from( " +
				"											SELECT SUM(nvl (HHV_NHV_BILL,0)) HHV_NHV_BILL, SUBSTR (CUSTOMER_ID, 3, 2) CATEGORY,ACCOUNT_NO,COLLECTION_DATE  " +
				"											FROM SUMMARY_MARGIN_PB pb, BILL_COLLECTION_METERED bcm  " +
				"											WHERE     pb.bill_id = BCM.BILL_ID " +wClause2+
				"											AND ACCOUNT_NO='"+account_no+"' " +
				"											GROUP BY SUBSTR (CUSTOMER_ID, 3, 2),ACCOUNT_NO,COLLECTION_DATE " +
				"											union all " +
				"											select sum(nvl(collected_hhv_nhv,0)) HHV_NHV_BILL,substr(sa.customer_id,3,2) Category,ACCOUNT_NO,COLLECTION_DATE from sales_adjustment sa,bill_collection_metered bcm " +
				"											where sa.bill_id=bcm.bill_id  " +wClause2+
				"											AND ACCOUNT_NO='"+account_no+"' " +
				"											GROUP BY substr(sa.customer_id,3,2),ACCOUNT_NO,COLLECTION_DATE) " +
				"											group by CATEGORY,ACCOUNT_NO,COLLECTION_DATE) " +
				"                                           tab2 " +
				"                                     WHERE tab1.cat = tab2.CATEGORY(+) " +
				"                                     and tab1.ACCOUNT_NO=tab2.ACCOUNT_NO(+) " +
				"                                     and tab1.TRANS_DATE=tab2.COLLECTION_DATE(+))) " +
	...
	try {
		...
		String transaction_sql="select TO_CHAR (TRANS_DATE, 'dd-mm-rrrr') TRANS_DATE, TO_Date (TRANS_DATE, 'dd-mm-rrrr') TRANS_DATE1,PARTICULARS,DEBIT,ACTUAL_REVENUE,METER_RENT,SURCHARGE,NULL Miscellaneous,CREDIT,null interest from( " +
				"select TRANS_DATE,PARTICULARS,SUM(DEBIT) DEBIT,SUM(CREDIT)CREDIT,SUM (ACTUAL_REVENUE)ACTUAL_REVENUE,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE from( " +
				"SELECT LAST_DAY(to_date(TRANS_DATE,'dd-mm-rrrr')) TRANS_DATE,PARTICULARS,SUM(DEBIT) DEBIT,SUM(CREDIT)CREDIT,SUM (ACTUAL_REVENUE)ACTUAL_REVENUE,SUM (METER_RENT) METER_RENT,SUM (SURCHARGE) SURCHARGE  " +
				"FROM BANK_ACCOUNT_LEDGER  " +
				"WHERE TRANS_TYPE IN (2,4)  " +
				"AND CREDIT > 0  " +
				"AND ACCOUNT_NO = '"+account_no+"' " +wClause+
				" GROUP BY TRANS_DATE, PARTICULARS) " +
				" group by TRANS_DATE, PARTICULARS) order by PARTICULARS,TRANS_DATE1 " ;
