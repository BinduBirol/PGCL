			...
			<div class="w-box">
				...
				<div class="w-box-content" style="padding: 10px;" id="content_div">						
						...
						<div class="row-fluid" id="sales_adjustment_div">							
							 <div class="span6" id="fromDateSpan">
				    			<label id="fromDateLabel" style="width: 48%">Effective Date</label>
								<input type="text" name="effective_date" id="effective_date" style="width: 45%" />
				  			</div>
						 ...
						 <div class="row-fluid">
							  <div class="span12" id="bill_amount">
				    			<label id="billAmt" style="width: 23.5%">Bill Amount</label>
								<input type="text" name="bill" id="bill" style="width: 73%" onkeyup="calBillAmount()" />
				  			</div>
				  			</div>
							  <div class="row-fluid">
								<div class="span12">
							 ...
							 <div class="row-fluid">
								<div class="span12">
									<label style="width: 23.5%">HHV/NHV Amount</label>	
									<input type="text" style="width: 73%"  id="hhv_amount" onkeyup="calBillAmount()"/>									
							  	</div>	
							 </div>
							 ...
							 <div class="row-fluid">
								<div class="span12">
									...
									<textarea rows="3" cols="" style="width: 73%;" id="remarks"></textarea>									
...
$("#unpaid_bill_grid").jqGrid($.extend(true, {}, scrollPagerGridOptions, {
	url: jsEnum.GRID_RECORED_FETCHER+'?service=org.pgcl.models.BillingService&method=getBillListNonMeterSalesAdjustment',
   	jsonReader: {
            repeatitems: false,
            id: "bill_id"
	},
...
function saveSalesAdjustment(){
	$.ajax({
		    url: 'saveSalesAdjustmentInfo.action',
		    type: 'POST',
		    data: {customer_id:$("#customer_id").val(),bill_month:$("#bill_month").val(),bill_year:$("#bill_year").val(),issue_date:$("#issue_date").val(),due_date:$("#due_date").val(), bill_amount:$("#bill").val(),surcharge_amount:$("#surcharge_amount").val(),meter_rent:$("#meter_amount").val(),hhv_amount:$("#hhv_amount").val(),total_consumption:$("#t_consumption").val(),payment_status:$("#payment_status").val(),effective_date:$("#effective_date").val()},
		    cache: false,
		    success: function (response) {
		    	$.jgrid.info_dialog(response.dialogCaption,response.message,jqDialogClose,jqDialogParam);	
		    	reloadGrid("unpaid_bill_grid");
...
function calcuateBillAmount(){
	...
	var hhvAmt = $("#hhv_amount").map(function(){return $(this).val();}).get();
	var totalAmount=0;
		totalAmount=parseFloat(billAmt)+parseFloat(surchargeAmt)+parseFloat(meterRentAmt)+parseFloat(hhvAmt);
...
function calBillAmount(){	
	var totalAmount = parseFloat($("#bill").val(),10)+parseFloat($("#surcharge_amount").val(),10)+parseFloat($("#meter_amount").val(),10)+parseFloat($("#hhv_amount").val(),10);
