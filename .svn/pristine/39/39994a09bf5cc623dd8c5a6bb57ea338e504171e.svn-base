...
import org.pgcl.enums.NumberToWordConversion;
...
//					"ORDER BY CATEGORY_ID " ;
	...
	private ArrayList<VatRebateITFFDTO> getFreedomFighterInfo(){
		...
		try {
			...
			String disconnection_info_sql="SELECT AA.CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CATEGORY_NAME,BILLED_AMOUNT,DOUBLE_BURNER_QNT,DEBIT FROM( " +
					"select CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CATEGORY_NAME,sum(nvl(BILLED_AMOUNT,0)) BILLED_AMOUNT,max(DOUBLE_BURNER_QNT) DOUBLE_BURNER_QNT from bill_non_metered " +
					"where FF_QUOTA='Y' " +
					"AND BILL_YEAR||lpad(bill_month,2,0) between "+firstPart+" and "+sndtPart+" " +
					"group by CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CATEGORY_NAME) AA, " +
					"(select CUSTOMER_ID,sum(nvl(COLLECTED_BILL_AMOUNT,0)) DEBIT from bill_collection_non_metered " +
					"where account_no='1' " +
					"AND collection_date between to_date('"+from_date+"','dd-mm-yyyy') AND to_date('"+to_date+"','dd-mm-yyyy') " +
					...
					/*"select BAL.CUSTOMER_ID CUSTOMER_ID,CUSTOMER_NAME,sum(DEBIT) DEBIT,CUSTOMER_CATEGORY_NAME,sum(BILLED_AMOUNT) BILLED_AMOUNT,max(DOUBLE_BURNER_QNT) DOUBLE_BURNER_QNT from BANK_ACCOUNT_LEDGER BAL,BILL_COLLECTION_NON_METERED BCM,BILL_NON_METERED BM "+
	...
	private ArrayList<VatRebateITFFDTO> getIndividualInfo(){
		...
		try {
			...
			String disconnection_info_sql="SELECT aa.BILL_ID,CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CATEGORY_NAME,BILL_MONTH,BILL_YEAR,ACTUAL_GAS_CONSUMPTION,BILLED_AMOUNT PAYABLE_AMOUNT,TAX_AMOUNT,CHALAN_NO,to_char(CHALAN_DATE,'dd-mm-yyyy') CHALAN_DATE from( " +
					"select BILL_ID,PAYABLE_AMOUNT,TAX_AMOUNT,CHALAN_NO,CHALAN_DATE from bill_collection_metered " +
					"WHERE customer_id='"+customer_id+"' " +
					"AND TAX_AMOUNT<>0 " +
					"AND Collection_date BETWEEN TO_DATE('"+from_date+"','dd-MM-yyyy') AND TO_DATE('"+to_date+"','dd-MM-yyyy')) aa, " +
					"(SELECT BILL_ID,CUSTOMER_ID,CUSTOMER_NAME,CUSTOMER_CATEGORY_NAME,BILL_MONTH,BILL_YEAR,ACTUAL_GAS_CONSUMPTION,BILLED_AMOUNT from bill_metered " +
					"WHERE customer_id='"+customer_id+"' ) bb " +
					"where AA.BILL_ID=BB.BILL_ID " +
