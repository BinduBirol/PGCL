...
public class NonMeterReconnectionReport extends BaseAction {
	...
	private void getMeterCustomerReconnectionInfo(Document document) throws DocumentException
	{
		...
		for(int i=0;i<expireListSize;i++)
		{
			...
			ptable.setWidths(new float[]{15,75,25,25,25,30,30,30,30,30});
			...
			pcell=new PdfPCell(new Paragraph("Minimum Load up to Reconn. Date",ReportUtil.f9B));
			pcell.setRowspan(2);
			pcell.setMinimumHeight(18f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
	...
	private void getNonMeterCustomerReconnectionInfo(Document document) throws DocumentException
	{
		...
		for(int i=0;i<expireListSize;i++)
		{
			...
			pcell = new PdfPCell(new Paragraph(reconnectionDto.getCustomer_id(),ReportUtil.f8));
			pcell.setMinimumHeight(16f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(String.valueOf(reconnectionDto.getBurner_qnt()),ReportUtil.f8));
			pcell.setMinimumHeight(16f);
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(reconnectionDto.getPertial_bill()),ReportUtil.f8));
			pcell.setMinimumHeight(16f);
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			ptable.addCell(pcell);
	...
	//PreparedStatement ps1=null;
		...
		try {
			...
			String defaulterSql="SELECT bqc.CUSTOMER_ID, " +
					...
					"       bqc.NEW_RECONN_QNT_4M_PERMANENT BURNER_QNT, " +
					...
					"       MCC.CATEGORY_NAME, " +
					"       cpi.Full_name, ca.ADDRESS_LINE1 ADDRESS, " +
					"       bqc.NEW_RECONN_QNT_4M_TEMPORARY BURNER_QNT, " +
					"       TO_CHAR (bqc.EFFECTIVE_DATE, 'dd-MM-YYYY') EFFECTIVE_DATE, " +
					...
					"       MCC.CATEGORY_NAME, " +
					"       cpi.Full_name, ca.ADDRESS_LINE1 ADDRESS, " +
					"       bqc.NEW_RECONN_QNT_4M_TEMP_HALF BURNER_QNT, " +
					"       TO_CHAR (bqc.EFFECTIVE_DATE, 'dd-MM-YYYY') EFFECTIVE_DATE, " +
