...
import org.pgcl.reports.masterData.CustomerCategory;
...
public class MonthlySalesStatement extends BaseAction {
	...
	public String execute() throws Exception {
		...
		String fileName=String.valueOf(Month.values()[Integer.valueOf(bill_month)-1])+"/"+bill_year+"SalesStatement"+String.valueOf(Area.values()[Integer.valueOf(loggedInUser.getArea_id())-1])+".pdf";
	...
	private static void area_wise(Document document) throws DocumentException
	{
		...
		try {
			...
			String sql1="select MCC.CATEGORY_ID,decode(SR.IS_METER,1,substr(CATEGORY_NAME,1,length(CATEGORY_NAME)-7),substr(CATEGORY_NAME,1,length(CATEGORY_NAME)-7)||' (Non-Meter)') CATEGORY_NAME" +
	...
	private static void category_wise(Document document) throws DocumentException
	{
		...
		datatable1.setWidthPercentage(100);
		datatable1.setWidths(new float[] {15,70,25,30,40,30,30,40,20,40,30,30,30,40});
		...
		try {
			...
			String sql1="select MCC.CATEGORY_ID,decode(SR.IS_METER,1,substr(CATEGORY_NAME,1,length(CATEGORY_NAME)-7),substr(CATEGORY_NAME,1,length(CATEGORY_NAME)-7)||' (Non-Meter)') CATEGORY_NAME" +
						...
						"AND SR.IS_METER=1 AND substr(SR.customer_id,3,2)=MCC.CATEGORY_ID and substr(SR.customer_id,1,2)=?"+ 
