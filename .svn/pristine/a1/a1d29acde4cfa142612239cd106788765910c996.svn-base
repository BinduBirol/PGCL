...
public class BGFCLPayableReport extends BaseAction {
	...
	private ArrayList<JournalVoucherDTO>getBGFCLCredit(){
		...
		try {
			...
			String defaulterSql="SELECT TO_CHAR (TRANS_DATE, 'dd-mm-yyyy') T_DATE,TRANS_DATE, COLLECTION_MONTH, COLLECTION_YEAR,NETPAID,INCOMETEX,MONTH,YEAR,WELLHEAD,SD,VAT,VATREBATE,RATIO From( " +
					...
					"(select  bm.month,bm.year, bm.vat/(bm.vat+sm.vat) ratio " +
					"from MARGIN_BGFCL bm, MARGIN_SGFL sm " +
					"where bm.month=sm.month and bm.year=sm.year " +
					"and to_number(bm.YEAR||lpad(bm.MONTH,2,0)) between "+firstPart+" and "+secondPart+") tm2 " +
					...
					"decode(PARTICULARS,'BGFCL',CREDIT,0) netpaid, " +
					"decode(PARTICULARS,'BGFCL Income Tax',CREDIT,0) incometax " +
					...
					"and PARTICULARS in( 'BGFCL','BGFCL Income Tax') " +
