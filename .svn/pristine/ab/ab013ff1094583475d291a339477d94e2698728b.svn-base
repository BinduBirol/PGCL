	...
	public String execute() throws Exception
	{
		...
		try{
			...
			if(report_for.equals("month_wise")){
				headLine="For the month of "+Month.values()[Integer.valueOf(bill_month)-1]+" "+bill_year;
...
//			}
			...
			securityAmount=bankBookJVList.get(38).getCredit();
			...
			double totalCredit=powerCredit+cngCredit+capCredit+indCredit+comCredit+domCredit+sPowerCredit+sCapCredit+sCngCredit+sIndCredit+
							   sComCredit+sDomCredit+mPowerCredit+mCapCredit+mCngCredit+mIndCredit+mComCredit+mDomCredit+interestIncome+hhvNHV+securityAmount;
			...
			pcell = new PdfPCell(new Paragraph("Bank A/c STD "+accountInfo.getAccount_no()+", "+accountInfo.getBranch_name()+", "+accountInfo.getBank_name()+", "+accountInfo.getAccount_name(),ReportUtil.f9));
			...
			if(str.contains("INDUSTRIAL")){
				...
				pcell = new PdfPCell(new Paragraph("Security Amount",ReportUtil.f9));
				pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				jvTable.addCell(pcell);
				...
				pcell = new PdfPCell(new Paragraph(taka_format.format(securityAmount),ReportUtil.f9));
				pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				jvTable.addCell(pcell);
				...
				pcell = new PdfPCell(new Paragraph("Account Receivable Commercial(Security)",ReportUtil.f9));
				pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				jvTable.addCell(pcell);
				...
				pcell = new PdfPCell(new Paragraph("Account Receivable Domestic(Security)",ReportUtil.f9));
				pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
				jvTable.addCell(pcell);
...
//			pcell=new PdfPCell(new Paragraph("PASHCHIMANCHAL GAS COMPANY LIMITED", ReportUtil.f9B));
//			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
//			pcell.setBorder(0);	
//			mTable1.addCell(pcell);
			...
			pcell=new PdfPCell(new Paragraph("Journal Voucher of Fund Transfer",ReportUtil.f11B));
			pcell.setMinimumHeight(18f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			pcell.setBorder(0);
			...
			pcell = new PdfPCell(new Paragraph("Bank A/c"+accountInfo.getBank_name()+","+accountInfo.getBranch_name()+","+" STD-"+accountInfo.getAccount_no()+", "+accountInfo.getAccount_name(),ReportUtil.f9));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			transferTable.addCell(pcell);
			...
			headerTable2.setWidths(new float[] {
				5,90,5
			});
			...
			pcell= new PdfPCell(new Paragraph(""));
			pcell.setBorder(0);
			headerTable2.addCell(pcell);
			
			PdfPTable mTable2=new PdfPTable(1);
			mTable2.setWidths(new float[]{100});
//			pcell=new PdfPCell(new Paragraph("PASHCHIMANCHAL GAS COMPANY LIMITED", ReportUtil.f9B));
//			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
//			pcell.setBorder(0);	
//			mTable1.addCell(pcell);
			...
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setBorder(0);
			mTable2.addCell(pcell);

			Chunk chunk5 = new Chunk("Revenue Section :",ReportUtil.f8B);
			Chunk chunk6 = new Chunk(String.valueOf(Area.values()[Integer.valueOf(loggedInUser.getArea_id())-1]),ReportUtil.f9B);
			Paragraph pp1 = new Paragraph(); 
			pp1.add(chunk5);
			pp1.add(chunk6);
			pcell=new PdfPCell(pp1);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setBorder(0);
			mTable2.addCell(pcell);
			...
			PdfPTable headLinetable2 = null;
			headLinetable2 = new PdfPTable(3);
			headLinetable2.setWidthPercentage(100);
			headLinetable2.setWidths(new float[]{10,80,10});
			...
			pcell = new PdfPCell(new Paragraph("",ReportUtil.f8));
			pcell.setBorder(0);
			headLinetable2.addCell(pcell);
			
			pcell=new PdfPCell(new Paragraph("Journal Voucher of Fund Receive",ReportUtil.f11B));
			pcell.setMinimumHeight(18f);
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
			pcell.setBorder(0);
	...
	private ArrayList<JournalVoucherDTO>getBankBook_AcJV(){
		...
		try {
			...
			String defaulterSql="SELECT PARTICULARS, " +
					...
					"                                       WHERE     trans_type in ( 1,7)  " +
					"                                             AND BANK_ID = '"+bank_id+"' " +
					"                           AND BRANCH_ID = '"+branch_id+"' " +
					"                           AND ACCOUNT_NO = '"+account_no+"' " +wClause+
					"                                    GROUP BY SUBSTR (bal.CUSTOMER_ID, 3, 2)) tab, " +
					"                                    (SELECT sum(nvl (HHV_NHV_BILL,0)) HHV_NHV_BILL,CATEGORY_ID,ACCOUNT_NO from(  " +
					"							SELECT SUM(nvl (HHV_NHV_BILL,0)) HHV_NHV_BILL, SUBSTR (CUSTOMER_ID, 3, 2) CATEGORY_ID,ACCOUNT_NO,COLLECTION_DATE  " + 
					...
					"							union all  " +
					"							select sum(nvl(collected_hhv_nhv,0)) HHV_NHV_BILL,substr(sa.customer_id,3,2) CATEGORY_ID,ACCOUNT_NO,COLLECTION_DATE from sales_adjustment sa,bill_collection_metered bcm " + 
					"							where sa.bill_id=bcm.bill_id   " +
					"                           AND BANK_ID = '"+bank_id+"' " +
					"                           AND BRANCH_ID = '"+branch_id+"' " +
					"							AND ACCOUNT_NO='"+account_no+"'  "  +wClause2+
					"							GROUP BY substr(sa.customer_id,3,2),ACCOUNT_NO,COLLECTION_DATE)  " +
					"							group by CATEGORY_ID,ACCOUNT_NO ) tab2, " +
					...
					"SELECT SUM(nvl (HHV_NHV_BILL,0)) Credit, SUBSTR (CUSTOMER_ID, 3, 2) CATEGORY_ID,ACCOUNT_NO,COLLECTION_DATE  " + 
					"							FROM SUMMARY_MARGIN_PB pb, BILL_COLLECTION_METERED bcm  " + 
					"							WHERE     pb.bill_id = BCM.BILL_ID  " +
					"                           AND BANK_ID = '"+bank_id+"' " +
					"                           AND BRANCH_ID = '"+branch_id+"' " +
					...
					"							GROUP BY SUBSTR (CUSTOMER_ID, 3, 2),ACCOUNT_NO,COLLECTION_DATE  " +
					"							union all  " +
					"							select sum(nvl(collected_hhv_nhv,0)) Credit,substr(sa.customer_id,3,2) CATEGORY_ID,ACCOUNT_NO,COLLECTION_DATE from sales_adjustment sa,bill_collection_metered bcm " + 
					"							where sa.bill_id=bcm.bill_id   " +
					"                           AND BANK_ID = '"+bank_id+"' " +
					"                           AND BRANCH_ID = '"+branch_id+"' " +
					"							AND ACCOUNT_NO='"+account_no+"'  "  +wClause2+
					...
					"       CATEGORY_NAME, " +
					"       CATEGORY_TYPE from ( " +
					"            SELECT 'Security' AS PARTICULARS, " +
					"               NULL CATEGORY_ID, " +
					"               SUM (nvl(DEBIT,0)) CREDIT,null CATEGORY_NAME,null CATEGORY_TYPE " +
					"          FROM bank_account_ledger " +
					"         WHERE     BANK_ID = '"+bank_id+"' " +
					"                           AND BRANCH_ID = '"+branch_id+"' " +
					"                           AND ACCOUNT_NO = '"+account_no+"' " +wClause+
	...
	private ArrayList<JournalVoucherDTO>getTransfer_AcJV(){
		...
		try {
			...
			String defaulterSql="select sum(nvl(CREDIT,0)) CREDIT,PARTICULARS from bank_account_ledger " +
					"where trans_type=4 " +
					"AND BANK_ID='"+bank_id+"' " +
					"AND BRANCH_ID='"+branch_id+"' " +
					"AND ACCOUNT_NO='"+account_no+"' " +wClause+
	...
	private ArrayList<JournalVoucherDTO>getReceive_AcJV(){
		...
		try {
			...
			String defaulterSql="select sum(nvl(DEBIT,0)) DEBIT,PARTICULARS from bank_account_ledger " +
					"where trans_type=4 " +
					"AND BANK_ID='"+bank_id+"' " +
					"AND BRANCH_ID='"+branch_id+"' " +
					"AND ACCOUNT_NO='"+account_no+"' " +wClause+
