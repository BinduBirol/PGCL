...
function changeReadingFormMode(mode){
	var always_disable_fields=["meter_rent","prev_reading_date","prev_reading","curr_reading_date","difference","hhv_nhv","rate","measurement_type_name","total_consumption","min_load","max_load"];
...
{	
	...
	if(reading_purpose==2){
		//General Billing
		if($("#measurement_type_str").val() == 1 || $("#customer_category").val()==11 || $("#customer_category").val()==12){
...
function validateMeterReadingInfo(){
	var reading_purpose=$("#reading_purpose_str").val();
	//alert(reading_purpose);
	var isValid=false;
	
	 if($("#measurement_type_name").val()=="EVC" || $("#customer_category").val()=='11' || $("#customer_category").val()=='12'||reading_purpose==8){
