...
import org.pgcl.dto.ClarificationDTO;
import org.pgcl.dto.CustomerListDTO;
	...
	private HashMap<String, Integer> getCustomerList(){
		...
		try {
			String transaction_sql=	"select aa.CATEGORY_ID,NVL(bb.COUNT,0) COUNT,NVL(BB.ISMETERED,1) ISMETERED from mst_customer_category aa,(  " +
					"select CATEGORY_ID,ISMETERED,count(CUSTOMER_ID) COUNT from (SELECT cc.customer_id,  " +
					"          STATUS,  " +
					"          SUBSTR (cc.customer_id, 3, 2) Category_id,  " +
					"          ISMETERED  " +
					"     FROM ( SELECT bqc.CUSTOMER_ID CUSTOMER_ID                     " +
					...
					"                                    AND effective_date <=  " +
					"                                           TO_DATE ('31-12-"+calender_year+"', 'dd-mm-YYYY')  " +
					...
					"             FROM bill_metered  " +
					"            WHERE bill_year = "+calender_year+" AND SUBSTR (customer_id, 1, 2) = '"+area+"') tbl,  " +
					"          customer_connection cc  " +
					"    WHERE tbl.customer_id = cc.customer_id) where substr(customer_id,1,2)='"+area+"' " +
					...
					/*"select aa.CATEGORY_ID,NVL(bb.COUNT,0) COUNT,NVL(BB.ISMETERED,1) ISMETERED from mst_customer_category aa,( " +
					"select CATEGORY_ID,ISMETERED,count(CUSTOMER_ID) COUNT from (SELECT cc.customer_id, " +
					"          STATUS, " +
					"          SUBSTR (cc.customer_id, 3, 2) Category_id, " +
					"          ISMETERED " +
					...
					"            WHERE bill_year = "+calender_year+" " +
					"           UNION " +
					"           SELECT DISTINCT (customer_id) " +
					"             FROM bill_metered " +
					"            WHERE bill_year = "+calender_year+") tbl, " +
					"          customer_connection cc " +
					"    WHERE tbl.customer_id = cc.customer_id) where substr(customer_id,1,2)=?"+
					" group by CATEGORY_ID,ismetered) bb " +
					" where aa.CATEGORY_ID=bb.CATEGORY_ID(+) " +
					" order by  aa.CATEGORY_ID desc,ISMETERED desc " ;*/
									          ...
									          "  FROM (SELECT CH.CUSTOMER_ID, SUBSTR (CH.CUSTOMER_ID, 3, 2) CATEGORY_ID,CC.ISMETERED "+
									           ...
									           "        WHERE CH.CUSTOMER_ID=cc.customer_id "+
									                 "   AND to_char(ISSUE_DATE,'yyyy') || LPAD (to_char(ISSUE_DATE,'MM'), 2, 0) BETWEEN ? AND ?"+
									                  " AND SUBSTR (CH.CUSTOMER_ID, 1, 2)=? AND CALENDER_YEAR = '"+calender_year+"')"+
									       " GROUP BY CATEGORY_ID,ISMETERED) bb "+
									"  WHERE aa.CATEGORY_ID = bb.CATEGORY_ID(+) "+
									"  order by  aa.CATEGORY_ID desc,ISMETERED desc ";
