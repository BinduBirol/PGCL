...
public class DWMBReport extends BaseAction {
	private static final long serialVersionUID = 1L;
	ArrayList<JournalVoucherDTO>dwmbList = new ArrayList<JournalVoucherDTO>();
	...
	public String execute() throws Exception
	{
		...
		String fileName="DWMB_Report.pdf";
...
//************************************ PGCL Logo *******************************************************
			...
			pcell=new PdfPCell(new Paragraph("Liabilities for DWMB Margin", ReportUtil.f11B));
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setBorder(0);
			mTable.addCell(pcell);
			...
			dwmbList=getBGFCLCredit();
			int listSize=dwmbList.size();
			
			for (int i = 0; i < listSize; i++) {
				bill = dwmbList.get(i).getBill();
				incomeTax=dwmbList.get(i).getIncomeTax();
				credit=dwmbList.get(i).getCredit();
				...
				if(i==0){
					...
					pcell = new PdfPCell(new Paragraph("01-07-"+dwmbList.get(i).getYear(),ReportUtil.f9B));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
				...
				if(bill>0.0){
					...
					pcell = new PdfPCell(new Paragraph(dwmbList.get(i).getTransactionDate(),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
					
					pcell = new PdfPCell(new Paragraph("By Bank,"+Month.values()[Integer.valueOf(dwmbList.get(i).getMonth())-1]+"-"+dwmbList.get(i).getYear(),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					...
					pcell = new PdfPCell(new Paragraph("To, DWMB Margin, "+Month.values()[Integer.valueOf(dwmbList.get(i).getMonth())-1]+"-"+dwmbList.get(i).getYear(),ReportUtil.f9));
	...
	private ArrayList<JournalVoucherDTO>getBGFCLCredit(){
		...
		try {
			...
			String defaulterSql="select to_char(TRANS_DATE) T_DATE,TRANS_DATE,MONTH,YEAR,BILL,incomeTax,WELLHEAD CREDIT from( " +
					"select TRANS_DATE,MONTH,YEAR,BILL,incomeTax,null WELLHEAD  from( " +
					"select TRANS_DATE,COLLECTION_MONTH MONTH,COLLECTION_YEAR YEAR,CREDIT BILL,round((CREDIT*5)/95,0) as incomeTax from MARGIN_ACCOUNT_PAYABLE_DTL MA, bank_account_ledger BA " +
					"where BA.TRANS_ID = MA.TRANS_ID  " +
					"AND trans_type=6 " +
					"AND PARTICULARS = 'DWELLHEAD' " +
					"AND TRANS_DATE between to_date('"+firstPart+"','dd/mm/yyyy') and to_date('"+secondPart+"','dd/mm/yyyy')) " +
					"union all " +
					"select TRANS_DATE,MONTH,YEAR,null BILL,null incomeTax,WELLHEAD from( " +
					"select  LAST_DAY (TO_DATE (MB.MONTH || '-' || MB.YEAR, 'MM-YYYY')) TRANS_DATE,MB.MONTH MONTH ,MB.YEAR YEAR,(MB.DWELLHED+MS.DWELLHED) WELLHEAD from MARGIN_BGFCL MB,MARGIN_SGFL MS " +
