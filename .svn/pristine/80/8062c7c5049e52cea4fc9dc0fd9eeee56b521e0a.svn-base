...
public class SecurityBankBookJV extends BaseAction {
	...
	public String execute() throws Exception
	{
		...
		try{
			...
			othersIncome=bankBookJVList.get(0).getOthersIncome()+bankBookJVList.get(1).getOthersIncome()+
					 ...
					 bankBookJVList.get(10).getOthersIncome()+bankBookJVList.get(11).getOthersIncome()+bankBooCreditList.get(2).getCredit();
			...
			securityAmount=bankBookDebitList.get(2).getDebit();
			
			double totalDebit=totalCredit-(corporateTax+bankCharge+securityAmount);
			...
			pcell = new PdfPCell(new Paragraph("Security Adjustment Amount",ReportUtil.f9));
			pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
			jvTable.addCell(pcell);
	...
	private ArrayList<BankBookDTO>getBankBook_AcJVDebit(){
		...
		try {
			...
			String defaulterSql="select 'Corporate Tax' as PARTICULARS,sum(CREDIT) DEBIT from bank_account_ledger " +
					...
					"UNION ALL " +
					"SELECT 'Security Adjustment' AS PARTICULARS, SUM (CREDIT) DEBIT " +
					"  FROM bank_account_ledger " +
					"WHERE BANK_ID='"+bank_id+"' " +
					"AND BRANCH_ID='"+branch_id+"' " +
					"AND ACCOUNT_NO='"+account_no+"' "+
					"       AND trans_type = 666 " + wClause;
	...
	private ArrayList<BankBookDTO>getCredit_AcJV(){
		...
		try {
			...
			String defaulterSql="SELECT PARTICULARS, SUM (CREDIT) CREDIT " +
					"    FROM (SELECT 'Sale of Application/Bill Book' PARTICULARS, " +
					"                 SUM (NVL (DEBIT, 0)) CREDIT " +
					"            FROM bank_account_ledger " +
					"           WHERE         BANK_ID = '"+bank_id+"' " +
					"       AND BRANCH_ID = '"+branch_id+"' " +
					"       AND ACCOUNT_NO = '"+account_no+"' " +wClause+
					...
					"          UNION ALL " +
					"          SELECT 'Sale of Application/Bill Book' PARTICULARS, " +
					"                 SUM (NVL (DEBIT, 0)) CREDIT " +
					"            FROM bank_account_ledger " +
					"           WHERE         BANK_ID = '"+bank_id+"' " +
					"       AND BRANCH_ID = '"+branch_id+"' " +
					...
					"GROUP BY PARTICULARS " +
					"UNION ALL " +
					"SELECT 'Interest Income' AS Particulars, SUM (DEBIT) CREDIT " +
					"  FROM bank_account_ledger " +
					" WHERE        BANK_ID = '"+bank_id+"' " +
					"       AND BRANCH_ID = '"+branch_id+"' " +
					"       AND ACCOUNT_NO = '"+account_no+"' " +wClause+
					"       AND trans_type IN (5, 3) " +
					"       AND PARTICULARS NOT LIKE '%Sal%' " +
					"UNION ALL " +
					"SELECT 'Others Income' AS Particulars,SUM (CREDIT) CREDIT from( " +
					"SELECT  SUM (DEBIT) CREDIT " +
					"  FROM bank_account_ledger " +
					" WHERE     BANK_ID = '"+bank_id+"' " +
					"       AND BRANCH_ID = '"+branch_id+"' " +
					"       AND ACCOUNT_NO = '"+account_no+"' " +wClause+
					"       AND PARTICULARS LIKE ('%Sale of Condensade%') " +
					"union all " +
					"SELECT  SUM (DEBIT) CREDIT " +
					"  FROM bank_account_ledger " +
					" WHERE        BANK_ID = '"+bank_id+"' " +
					"       AND BRANCH_ID = '"+branch_id+"' " +
					"       AND ACCOUNT_NO = '"+account_no+"' " +wClause+
					"       AND PARTICULARS LIKE ('%Sale Duplicate Connection Card%') " +
					"union all " +
					"SELECT  SUM (DEBIT) CREDIT " +
					" FROM bank_account_ledger " +
					" WHERE        BANK_ID = '"+bank_id+"' " +
