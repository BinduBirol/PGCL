...
public class SecurityBankBook extends BaseAction {
	...
	public String execute() throws Exception
	{
		...
		try{
			...
			if(report_for.equals("month_wise")){
				...
				pcell.setBorder(0);
				headLinetable.addCell(pcell);
				pcell=new PdfPCell(new Paragraph("BANK BOOK OF SECURITY & OTHERS COLLECTION FOR THE PERIOD OF "+from_date+" TO "+to_date,ReportUtil.f11B));
				pcell.setMinimumHeight(18f);
				pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
				pcell.setVerticalAlignment(Element.ALIGN_MIDDLE);
				pcell.setBorder(0);
			...
			if(report_for.equals("date_wise")){
				openingBalance=getBankBookOpeningBalanceDatewise();
	...
	//PreparedStatement ps1=null;
		...
		try {
			...
			{
				wClause=" And to_char(DEPOSIT_DATE,'mm')="+collection_month+" and to_char(DEPOSIT_DATE,'YYYY')="+collection_year;
			}
	...
	//PreparedStatement ps1=null;
		...
		try {
			...
			String transaction_sql="SELECT * FROM ( " +
					"SELECT TO_CHAR(TRANS_DATE,'dd-MON-yyyy') TRANS_DATE, TRANS_DATE TRANS_DATE1, " +
					...
					"       AND TRANS_TYPE = 3 " +
					"       AND DEBIT <> 0 "+ 
					"       AND PARTICULARS not like 'Interest Income' " +wClause+" " +
					"UNION ALL   " +
					"SELECT TO_CHAR(TRANS_DATE,'dd-MON-yyyy') TRANS_DATE, TRANS_DATE TRANS_DATE1,  " +
					"       PARTICULARS,DEBIT totalcollection,   " +
					"       null otherincome,   " +
					...
					"       AND TRANS_TYPE = 3 " +
					"       AND DEBIT <> 0  " +
					"       AND PARTICULARS like 'Interest Income' " +wClause+" ) " +
	...
	//PreparedStatement ps1=null;
		...
		try {
			...
			String transaction_sql= "SELECT * FROM ( " +
									"SELECT TO_CHAR (TRANS_DATE,'dd-mm-yyyy') TRANS_DATE,TRANS_DATE TRANS_DATE1, " +
									"       PARTICULARS, " +
									"       DEBIT, " +
									"       credit " +
									"  FROM BANK_ACCOUNT_LEDGER " +
