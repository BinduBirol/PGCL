...
import org.pgcl.dto.DepositDtlDTO;
...
public class DepositService {
	...
	public ResponseDTO saveDeposit(String customer_id, DepositDTO deposit) {
		...
		try {
			...
			if (Integer.valueOf(deposit.getStr_deposit_type()) == DepositType.CASH_BANK
				...
				bank_account_stmt.setInt(2,
						BankAccountTransactionType.SECURITY_OTHER_DEPOSIT
								.getId());
	...
	public ResponseDTO editDeposit(DepositDTO deposit) {
		...
		String depositLedgerSql = " Update CUSTOMER_SECURITY_LEDGER Set SECURITY_AMOUNT=?,DEBIT=? Where DEPOSIT_ID=?";
		...
		try {
			...
			deposit_ledger_stmt.setString(3, deposit.getDeposit_id());
			deposit_ledger_stmt.execute();
	...
	public DepositDTO getDepositDetail(String deposit_id) {
		...
		try {
			...
			sql = "Select MST_DEPOSIT_TYPES.*,DTL_DEPOSIT.AMOUNT from MST_DEPOSIT_TYPES Left outer join DTL_DEPOSIT "
					+ "       ON MST_DEPOSIT_TYPES.TYPE_ID=DTL_DEPOSIT.TYPE_ID "
					+ "       AND STATUS=1  AND DEPOSIT_ID=? ORDER BY VIEW_ORDER ";
			stmt = conn.prepareStatement(sql);
			stmt.setString(1, deposit_id);
			r = stmt.executeQuery();
	...
	public ArrayList<DepositDTO> getCandidatesForBankGuaranteeExpire(int index,
		...
		if (sortFieldName != null && !sortFieldName.equalsIgnoreCase(""))
			orderByQuery = " ORDER BY " + sortFieldName + " " + sortOrder + " ";
		if (total == 0)
			sql = "Select deposit.deposit_id,deposit.customer_id,CUSTOMER_CATEGORY,CATEGORY_NAME,FULL_NAME ,bank_id,branch_id,account_no,null bank_name,null branch_name,null account_name,total_deposit,to_char(valid_to,'dd-MM-YYYY') valid_to,valid_to -trunc(sysdate) expire_in "
					+ " From MST_DEPOSIT deposit,CUSTOMER_PERSONAL_INFO CUSTOMER,CUSTOMER CUS,MST_CUSTOMER_CATEGORY MCC"
					+ " Where deposit.customer_id=CUSTOMER.CUSTOMER_ID"
					+ " AND CUS.CUSTOMER_ID=deposit.customer_id "
					+ " AND MCC.CATEGORY_ID=CUS.CUSTOMER_CATEGORY"
					+ " And Deposit_Type=1 And Deposit_Purpose=8 AND bank_id is null AND substr(deposit.customer_id,1,2)= '"+area_id+"' And (valid_to - TRUNC (SYSDATE))<=90  "
					+ (whereClause.equalsIgnoreCase("") ? "" : (" And ( "
							+ whereClause + ")")) + " " + orderByQuery;
		else
			sql = " Select * from ( "
					+ " Select rownum serial,tmp1.* from "
					+ " ( Select deposit.deposit_id,deposit.customer_id,CUSTOMER_CATEGORY,CATEGORY_NAME,FULL_NAME ,bank_id,branch_id,account_no,null bank_name,null branch_name,null account_name,total_deposit,to_char(valid_to,'dd-MM-YYYY') valid_to,valid_to -trunc(sysdate) expire_in "
					+ " From MST_DEPOSIT deposit,CUSTOMER_PERSONAL_INFO CUSTOMER,CUSTOMER CUS,MST_CUSTOMER_CATEGORY MCC "
					+ " Where deposit.customer_id=CUSTOMER.CUSTOMER_ID"
					+ " AND CUS.CUSTOMER_ID=deposit.customer_id "
					+ " AND MCC.CATEGORY_ID=CUS.CUSTOMER_CATEGORY"
					+ " And Deposit_Type=1 And Deposit_Purpose=8 AND substr(deposit.customer_id,1,2)= '"+area_id+"' And (valid_to - TRUNC (SYSDATE))<=90  "
					+ (whereClause.equalsIgnoreCase("") ? "" : (" And ( "
							+ whereClause + ")")) + " " + orderByQuery
					+ "    )tmp1 " + "    )tmp2   "
					+ "  Where serial Between ? and ? ";
