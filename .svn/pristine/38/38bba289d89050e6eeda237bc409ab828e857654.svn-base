...
public class VatRevateJV extends BaseAction {
	private static final long serialVersionUID = 1L;
	ArrayList<JournalVoucherDTO>vatRebateJVList = new ArrayList<JournalVoucherDTO>();
	...
	public String execute() throws Exception
	{
		...
		String fileName="VAT_Rebate_JV.pdf";
		...
		try{
			...
			if(report_for.equals("month_wise")){
				headLine="Journal Voucher of VAT Rebate for the month "+Month.values()[Integer.valueOf(bill_month)-1]+" "+bill_year;
			}else if(report_for.equals("fiscal_wise")){
				headLine="Journal Voucher of VAT Rebate FY :"+collection_year;
			}else if(report_for.equals("date_wise")){
				headLine="Journal Voucher of VAT Rebate From "+from_date+" to "+to_date;
			}
			...
			vatRebateJVList=getVATRebateJV();
			
			double bgfclDebit=vatRebateJVList.get(0).getDebit();
			double sgflDebit=vatRebateJVList.get(1).getDebit();
			
			double bgfclCredit=vatRebateJVList.get(2).getCredit();
			double sgflCredit=vatRebateJVList.get(3).getCredit();
			
			
			pcell = new PdfPCell(new Paragraph(vatRebateJVList.get(0).getParticulars(),ReportUtil.f9));
			...
			pcell = new PdfPCell(new Paragraph(vatRebateJVList.get(1).getParticulars(),ReportUtil.f9));
			pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
			jvTable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph("Accounts Receivable(Cap. Power)(80% VAT Rebate from )"+vatRebateJVList.get(2).getParticulars(),ReportUtil.f8));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			jvTable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(bgfclCredit),ReportUtil.f9));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			jvTable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph("Accounts Receivable(Industrial)(80% VAT Rebate from )"+vatRebateJVList.get(3).getParticulars(),ReportUtil.f8));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			jvTable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(sgflCredit),ReportUtil.f9));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			jvTable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(vatRebateJVList.get(0).getDebit()+vatRebateJVList.get(1).getDebit()),ReportUtil.f9B));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			jvTable.addCell(pcell);
			...
			pcell = new PdfPCell(new Paragraph(taka_format.format(vatRebateJVList.get(2).getCredit()+vatRebateJVList.get(3).getCredit()),ReportUtil.f9B));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			jvTable.addCell(pcell);
			...
			if(report_for.equals("date_wise")){
				...
				pcell = new PdfPCell(new Paragraph("Being the amount 80% VAT Rebatted from Gas bill of Captive Power & Industrial Categories for the Period: "+from_date+" to "+to_date+" taken into accounts through this Journal Voucher.",ReportUtil.f8));
				pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
				pcell.setColspan(7);
				jvTable.addCell(pcell);
			}else if(report_for.equals("month_wise")){
				pcell = new PdfPCell(new Paragraph("Being the amount 80% VAT Rebatted from Gas bill of Captive Power & Industrial Categories for the month : "+Month.values()[Integer.valueOf(bill_month)-1]+" "+bill_year+" taken into accounts through this Journal Voucher.",ReportUtil.f8));
				pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
				pcell.setColspan(7);
				jvTable.addCell(pcell);
			}else if(report_for.equals("fiscal_wise")){
				pcell = new PdfPCell(new Paragraph("Being the amount 80% VAT Rebatted from Gas bill of Captive Power & Industrial Categories for the FY : "+collection_year+" taken into accounts through this Journal Voucher.",ReportUtil.f8));
				pcell.setHorizontalAlignment(Element.ALIGN_LEFT);
				pcell.setColspan(7);
				jvTable.addCell(pcell);
			}
	...
	private ArrayList<JournalVoucherDTO>getVATRebateJV(){
		...
		try {
			...
			if(report_for.equals("date_wise")){
				...
				wClause=" AND bm.BILL_YEAR||lpad(bm.BILL_MONTH,2,0) between "+fromDate1+" and "+toDate1+" " ;
				wClause1=" AND bm.YEAR||lpad(bm.MONTH,2,0) between "+fromDate1+" and "+toDate1+" " ;
			}if(report_for.equals("month_wise")){
				wClause=" AND bm.BILL_YEAR="+bill_year+" AND bm.BILL_MONTH="+bill_month+" ";
				...
				wClause=" AND bm.BILL_YEAR||lpad(bm.BILL_MONTH,2,0) between "+firstPart+" and "+secondPart+" " ;
				wClause1=" AND bm.YEAR||lpad(bm.MONTH,2,0) between "+firstPart+" and "+secondPart+" " ;
			}
			
			
			String defaulterSql="SELECT NULL CUSTOMER_ID,FULL_NAME,null CAT,SUM(DEBIT) DEBIT,null CREDIT FROM( " +
					"SELECT 'Liabilities for GAS purchase(BGFCL)' FULL_NAME,(VATREBATE*RATIO) DEBIT FROM ( " +
					"SELECT  BILL_MONTH, BILL_YEAR, SUM (VAT_REBATE_AMOUNT) VATREBATE " +
					"FROM SUMMARY_MARGIN_PB sm, BILL_METERED bm " +
					"WHERE     SM.BILL_ID = bm.bill_id " +
					"AND NVL (VAT_REBATE_AMOUNT, 0) > 0 " +wClause+
					...
					")) tm2 " +
					"WHERE tm1.BILL_MONTH(+) = tm2.BILL_MONTH " +
					"AND tm1.BILL_YEAR(+) = tm2.BILL_YEAR) " +
					"GROUP BY FULL_NAME " +
					"UNION ALL " +
					"SELECT NULL CUSTOMER_ID,FULL_NAME,null CAT,SUM(DEBIT) DEBIT,NULL CREDIT FROM( " +
					"SELECT 'Liabilities for GAS purchase(SGFL)' FULL_NAME,(VATREBATE*RATIO) DEBIT FROM ( " +
					"SELECT  BILL_MONTH, BILL_YEAR, SUM (VAT_REBATE_AMOUNT) VATREBATE " +
					"FROM SUMMARY_MARGIN_PB sm, BILL_METERED bm " +
					"WHERE     SM.BILL_ID = bm.bill_id " +
					"AND NVL (VAT_REBATE_AMOUNT, 0) > 0 " +wClause+
					...
					"GROUP BY FULL_NAME " +
					"UNION ALL " +
					"SELECT CUSTOMER_ID,FULL_NAME,CAT,DEBIT,CREDIT FROM( " +
					"select CPI.CUSTOMER_ID,CPI.FULL_NAME,cat,DEBIT,CREDIT from( " +
					"SELECT bm.CUSTOMER_ID,CUSTOMER_CATEGORY_NAME cat,null DEBIT, SUM (VAT_REBATE_AMOUNT) CREDIT " +
					"FROM SUMMARY_MARGIN_PB sm, BILL_METERED bm " +
					"WHERE SM.BILL_ID = bm.bill_id " +
					"AND NVL (VAT_REBATE_AMOUNT, 0) > 0 " +wClause+
					"GROUP BY bm.CUSTOMER_ID,bm.CUSTOMER_CATEGORY_NAME) tab1,CUSTOMER_PERSONAL_INFO CPI " +
					"WHERE CPI.CUSTOMER_ID=tab1.CUSTOMER_ID) " ;
        	...
        	while(resultSet.next())
        	{
        		...
        		jDto.setParticulars(resultSet.getString("FULL_NAME"));
