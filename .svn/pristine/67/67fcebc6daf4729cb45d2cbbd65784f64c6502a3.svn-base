...
public class MonthlySalesSummary extends BaseAction {
	...
	Connection conn = ConnectionManager.getConnection();
	    ...
	    private  String[] areaList={"01","02","03","04","05","06","07","08","09","10"};
	...
	public String execute() throws Exception
	{
		...
		try{
			...
			pcell=new PdfPCell(new Paragraph("Sales Statement for the Month of ", ReportUtil.f11B));
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setBorder(0);
			mTable.addCell(pcell);					
			...
			for (int i = 0; i < listSize; i++) {
				...
				if(i==0){
					...
					datatable1.addCell(pcell);
					pcell=new PdfPCell(new Paragraph("SIRAJGANJ AREA",font3));
					pcell.setColspan(13);
					datatable1.addCell(pcell);
			...
			pcell=new PdfPCell(new Paragraph("Total Sales of SIRAJGANJ(A+B)=",ReportUtil.f9B));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			pcell.setColspan(2);
			datatable1.addCell(pcell);
			...
			for (int i = 0; i < listSizesum; i++) {
				...
				if(i==0){
					...
					pcell=new PdfPCell(new Paragraph("1",font3));
					summaryTable.addCell(pcell);
					pcell=new PdfPCell(new Paragraph("PGCL",font3));
					pcell.setColspan(13);
					summaryTable.addCell(pcell);
			...
			pcell=new PdfPCell(new Paragraph("Total Sales of PGCL(A+B)=",ReportUtil.f9B));
			pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
			pcell.setColspan(2);
			summaryTable.addCell(pcell);
	...
	private ArrayList<SalesDTO>getAreaWiseSalesSirajganj(){
		...
		try {
			...
			String defaulterSql="select MCC.CATEGORY_ID CATEGORY_ID,decode(CONN.ISMETERED,1,substr(CATEGORY_NAME,1,length(CATEGORY_NAME)-7),substr(CATEGORY_NAME,1,length(CATEGORY_NAME)-7)||' (Non-Meter)') CATEGORY_NAME" +
					...
					"and substr(SR.customer_id,3,2)=MCC.CATEGORY_ID and substr(SR.customer_id,1,2)='"+area+"'"+ 
			...
			ps1=conn.prepareStatement(defaulterSql);
