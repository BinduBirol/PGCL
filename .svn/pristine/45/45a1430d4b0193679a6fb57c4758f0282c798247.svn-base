...
public class JVBGFCLReport extends BaseAction {
	private static final long serialVersionUID = 1L;
	ArrayList<JournalVoucherDTO>bgfclList = new ArrayList<JournalVoucherDTO>();
	ArrayList<JournalVoucherDTO>bgfclDebitList = new ArrayList<JournalVoucherDTO>();
...
//************************************ PGCL Logo *******************************************************
			...
			img.scaleAbsolute(45, 59);
            img.setAbsolutePosition(100f, 510f); 			// Rotate
			//img.setAbsolutePosition(90f, 770f); 			// normal
			...
			pcell=new PdfPCell(new Paragraph("Liabilities for Wellhead, SD & VAT (BGFCL)", ReportUtil.f11B));
			pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
			pcell.setBorder(0);
			mTable.addCell(pcell);
			...
			for (int i = 0; i < listSize; i++) {
				bgfclAmt = bgfclList.get(i).getBgfcl();
				welHeadAmt=bgfclList.get(i).getWellHead();
				sdAmt=bgfclList.get(i).getSd();
				vatAmt=bgfclList.get(i).getVat();
				...
				if(bgfclAmt>0.0){
					balance=balance-bgfclList.get(i).getBgfcl();
				}else{
					balance=balance+welHeadAmt+sdAmt+vatAmt;
				}
				if(i==0){
					
					pcell = new PdfPCell(new Paragraph("01-07-"+bgfclList.get(i).getYear(),ReportUtil.f9B));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
				...
				if(bgfclAmt>0.0){
					...
					pcell = new PdfPCell(new Paragraph("By Bank,"+Month.values()[Integer.valueOf(bgfclList.get(i).getMonth())-1]+"-"+bgfclList.get(i).getYear(),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(bgfclList.get(i).getBgfcl()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					
					totalDebit=totalDebit+bgfclList.get(i).getBgfcl();
					...
					pcell = new PdfPCell(new Paragraph(bgfclList.get(i).getLastDay(),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_CENTER);
					jvTable.addCell(pcell);
					
					pcell = new PdfPCell(new Paragraph("To, Gas Purchase, "+Month.values()[Integer.valueOf(bgfclList.get(i).getMonth())-1]+"-"+bgfclList.get(i).getYear(),ReportUtil.f9));
					...
					pcell = new PdfPCell(new Paragraph(taka_format.format(bgfclList.get(i).getWellHead()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					
					totalWellHead=totalWellHead+bgfclList.get(i).getWellHead();
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(bgfclList.get(i).getSd()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					
					totalSD=totalSD+bgfclList.get(i).getSd();
					
					pcell = new PdfPCell(new Paragraph(taka_format.format(bgfclList.get(i).getVat()),ReportUtil.f9));
					pcell.setHorizontalAlignment(Element.ALIGN_RIGHT);
					jvTable.addCell(pcell);
					
					totalVAT=totalVAT+bgfclList.get(i).getVat();
					
					totalPurchase=bgfclList.get(i).getWellHead()+bgfclList.get(i).getSd()+bgfclList.get(i).getVat();
	...
	private ArrayList<JournalVoucherDTO>getBGFCLCredit(){
		...
		try {
			...
			String defaulterSql="select month,year, WELLHEAD, sd, vat,TOTAL_BGFCL, TO_CHAR(LASTDAY) LASTDAY from( " +
								"select month,year,null WELLHEAD, null sd,null vat,TOTAL_BGFCL,LAST_DAY(TO_Date(month||'-'||year, 'MM-YYYY')) LASTDAY " +
								"from GAS_PURCHASE_SUMMARY where year||lpad(month,2,0) between '"+firstPart+"' and '"+secondPart+"' " +
								"union all " +
								"select month,year,WELLHEAD, sd,vat, null TOTAL_BGFCL,LAST_DAY(TO_Date(month||'-'||year, 'MM-YYYY')) LASTDAY " +
        	...
        	while(resultSet.next())
        	{
        		...
        		jDto.setBgfcl(resultSet.getDouble("TOTAL_BGFCL"));
