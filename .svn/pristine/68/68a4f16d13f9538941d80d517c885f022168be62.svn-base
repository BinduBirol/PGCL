...
var addParams={
        ...
        addCaption: jqCaption.ADD_USER,
        url: jsEnum.CRUD_ACTION,
        serializeEditData: function (postdata) 
            {
    		if (postdata.id === '_empty')
    	        postdata.id = null;
                return {data: JSON.stringify(postdata), service:jsEnum.USER_SERVICE,  method:jsEnum.USER_ADD};
        },
        beforeShowForm: function( formId ) {
...
.jqGrid({
    ...
    colNames: ['Photo','', 'User Id','Password', 'User Name','Role','Area','Division','Department','Section', 'Designation','Division','District','Upazila','Email','Mobile','Status'],
    colModel: [{
        name: 'userImg',
        width: 40,
        align: 'center',
            ...
            success: function(data, status) {
                ...
                if (data == 'Not a Valid type') {
                    ...
                    if (navigator.appName == 'Opera') {
                        ...
                        document.getElementById('userImg').innerHTML = "";
                        document.getElementById('userImg').value = "/PGCL_WEB/" + data + "";
                        document.getElementById('userImg').src = "/PGCL_WEB/" + data + "";
                    }
                }
                 $("#upload").change(afu);
...
function resetPassword(userId)
{
	...
	$.ajax({
	    url: 'resetUserPassword.action?user.userId='+userId,
	    type: 'POST',
	    processData: false,
	    contentType:false,
	    async: true,
